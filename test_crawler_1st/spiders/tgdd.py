    ...
    def parse(self, response):
        if response.status == 200 and response.css('section::attr(class)').get() == 'type0':
            ...
            data = {
                ...
                'rating': response.css('div.toprt div.crt b::text').get(),
                'category': '/ '.join([
                ...
                'list_img_src': response.css('meta[itemprop="image"]::attr(content)').get(),
        ...
        yield from response.follow_all(
            css='a[href^="https://www.thegioididong.com/"]::attr(href), a[href^="/"]::attr(href)', callback=self.parse)
